...
-- 4. ASSIGNMENT → STUDENT mapping (must exist before assignments policy that refs it)
...
-- Now add the cross-reference policy on assignments (assignment_students now exists)
...
-- Now add the cross-reference policy on tests (test_students now exists)
